...
export default function drawerSceneCreator(scenes: Scenes): SceneDestructor {
  ...
  let floppyAnimation: gsap.core.Timeline | gsap.core.Tween = gsap
    .timeline({ delay: 3 })
    .to("#silver", { x: -30, y: -16 })
    .to("#silver", { x: 0, y: 0, duration: 0.1 });
  ...
  hoverHandler(document.querySelector("#floppy"), {
    ...
    leave: () => {
      floppyAnimation.kill();
      floppyAnimation = gsap.to("#silver", { x: 0, y: 0, duration: 0.1 });
